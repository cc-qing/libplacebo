...
static bool gl_tex_upload_from_va_surface(pl_gpu gpu, const struct pl_tex_transfer_params *params)
{
    const gl_funcs *gl = gl_funcs_get(gpu);
    pl_tex tex = params->tex;
    struct pl_tex_gl *tex_gl = PL_PRIV(tex);
    gl->BindTexture(tex_gl->target, tex_gl->texture);
    gl->EGLImageTargetTexture2DOES(tex_gl->target, params->va->egl_image);
    return true;
}
...
bool gl_tex_upload(pl_gpu gpu, const struct pl_tex_transfer_params *params)
{
    ...
    if (params->va)
        return gl_tex_upload_from_va_surface(gpu, params);
